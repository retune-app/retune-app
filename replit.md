...
- **Breathing Mode**: Offers various breathing techniques (Box, 4-7-8, Coherent, Energizing) with animated visuals, duration selection, binaural beats. Each technique has a subtle info icon that opens a detailed modal with description, science tip, and benefits list with icons. Breathe reminders include time-of-day-specific technique recommendations. Fullscreen layout: top bar (technique badge + always-visible audio controls + close button, z-index: 10), centered breathing rings (z-index: 1), bottom area with centered play/pause button (stop button absolutely positioned to its right) and stats row (Time Left, Progress, Cycles). Audio buttons always visible — dimmed at 40% opacity when not enabled. Haptic preference read from global Settings via AsyncStorage (`@settings/hapticFeedback`).
