...
## Recent Changes
- **v1.7.3 Mood Journey Restoration (Feb 19, 2026)**: Restored two-step conversational mood check-in ("How are you feeling?" → "I want to feel...") with 6 starting moods and 6 target moods. Vibe system now operates silently behind the scenes — `resolveVibeFromMoodPair()` auto-maps mood pairs to vibeId for intelligent routing of breathing techniques, meditation styles, TTS params, and affirmation matching. Server `/api/mood-checkin` endpoint returns resolved vibeId + vibe config in response. Architecture: `shared/vibes.ts` (config + resolveVibeFromMoodPair), `server/vibe-engine.ts` (routing), `client/components/MoodCheckin.tsx` (restored 2-step UI with mood pair display in journey results). Renamed "Tags" to "Intentions" in UI. Added `vibeId` column to `journey_completions` table.
