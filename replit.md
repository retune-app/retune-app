...
## Recent Changes
- **v1.7.3 Vibe System (Feb 19, 2026)**: Major UX simplification replacing 2-step mood check-in with single-step "Vibe" selection (8 vibes: Reset, Chill, Locked In, Glow Up, In My Head, Steady, Fired Up, Heavy). Architecture: `shared/vibes.ts` (config module mapping vibes to TTS params, breathing techniques, meditation styles, affirmation matching, UI colors), `server/vibe-engine.ts` (routing module), `/api/vibe-checkin` endpoint. Vibe context flows into meditation scripts, TTS audio config, and AI daily greetings. Renamed "Tags" to "Intentions" in UI. Added `vibeId` column to `journey_completions` table. Fixed duplicate `addTagButton` style in CreateScreen.
