...
## Recent Changes (v1.7.1 Build 4 — February 16, 2026)
- **Google OAuth Redirect Fix**: Changed `GoogleSignInFallback` redirect URI from `getApiUrl()` (which resolved to the Replit dev domain) to `https://retuned.app` so it matches the authorized redirect URI in Google Cloud Console. Previous builds failed with "redirect_uri_mismatch" (Error 400). This fix is now baked directly into the EAS build's embedded bundle.
- **Google OAuth EAS Fix**: Added `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` and `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` to `eas.json` production env block so Google client IDs are baked into the native bundle at EAS build time. Build 1 failed with "Missing required parameter: client_id" because the env vars only existed in Replit.
...
- **Production Bundle Rebuild**: New production bundle (build `1771214945897-31740`) deployed with Google OAuth redirect fix and landing page updates.
- **Build Number Bump**: iOS `buildNumber` set to "4" in `app.json` for new EAS build + App Store submission.
