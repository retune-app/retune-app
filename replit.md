...
- **Guided Moments**: Ephemeral AI-generated personalized meditation audio (60-90 seconds) triggered after mood check-in. Uses OpenAI gpt-4o-mini for script generation with structured mindfulness format (opening, breathing guidance, visualization, affirmation anchoring, closing). Audio generated via Hume AI (Lotus voice, Female Meditation Guide) by default, with ElevenLabs for personal cloned voices. Includes word-by-word RSVP display synchronized with audio inside breathing rings, safety disclaimer, and replay option. Rate limited to 5/day per user. No database storage — fully ephemeral. Component: `client/screens/GuidedMomentScreen.tsx`. Endpoint: `POST /api/guided-moments/generate` (body: `{mood, timeOfDay, usePersonalVoice?, voiceId?}`). Fully immersive single-phase experience: auto-pairs mood-matched background music (skips selection), concentric breathing rings with RSVP words displayed inside the ring center, voice selector (Lotus/Sage/My Voice), sound switcher modal, auto-hiding controls after 3s of playback (tap to reveal), responsive portrait/landscape layout. Voice preference persisted via AsyncStorage.
